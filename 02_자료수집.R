...
loc <- read.csv("./00_code/sigun_code/sigun_code.csv", fileEncoding = "CP949")  #  지역코드
loc$code <- as.character(loc$code) # 행정구역명 문자 변환     
head(loc, 2) # 확인

#---# [3단계: 수집 기간 설정]

datelist <- seq(from = as.Date('2020-01-01'), # 시작
                to   = as.Date('2020-12-31'), # 종료
                by    = '1 month')            # 단위
...
total <- list()      # 거래내역 순차적 저장 임시 저장소
dir.create("01_raw_data") # 새로운 폴더 만들기

#---# [2단계: url 요청 - XML 응답]

for(i in 1:length(url_list)){   # 요청목록(url_list) 반복
  raw_data[[i]] <- xmlTreeParse(url_list[i], useInternalNodes = TRUE,encoding = "utf-8") # 응답결과 xml 저장
...
#---# [5단계: 응답내역 저장]
  ...
  path <- as.character(paste0("./01_raw_data/", region_nm, "_", month,".csv")) # 저장위치 설정
...
setwd(dirname(rstudioapi::getSourceEditorContext()$path))  # 작업폴더 설정
files <- dir("./01_raw_data")    # 폴더 내 모든 파일 이름 읽기
library(plyr)     # install.packages("plyr")
apt_price <- ldply(as.list(paste0("./01_raw_data/", files)), read.csv) # 모든 파일 하나로 결합
tail(apt_price, 2)  # 확인

#---# [2단계: 저장]

dir.create("./02_integrated")   # 새로운 폴더 생성
save(apt_price, file = "./02_integrated/02_apt_price.rdata") # 저장
write.csv(apt_price, "./02_integrated/02_apt_price.csv")   
